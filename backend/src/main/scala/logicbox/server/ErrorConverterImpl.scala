...
class ErrorConverterImpl[F, R, B, O](
  ...
  override def convert(stepId: String, error: Error): Option[OutputError] = error match {
    ...
    case FreshVarEscaped(boxId) => for {
      freshVar <- proofNavigator.get((proof, boxId), Location.conclusion.freshVar) // TODO: feeling bad about this 'conclusion'
      freshVarStr = actualExpToString(freshVar)
    } yield OutputError.FreshVarEscaped(stepId, boxId, freshVarStr)

    case RedefinitionOfFreshVar(originalBoxId) => for {
      freshVar <- proofNavigator.get((proof, stepId), Location.conclusion.freshVar)
      freshVarStr = actualExpToString(freshVar)
    } yield OutputError.RedefinitionOfFreshVar(stepId, originalBoxId, freshVarStr)
    ...
    case InvalidAssumption() => Some(OutputError.Simple(stepId, "InvalidAssumption"))
