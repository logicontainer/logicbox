...
) extends Proof[F2, R2, B2, Id]
{
  ...
  def getStep(id: Id): Option[Proof.Step[F2, R2, B2, Id]] = for {
    step <- inner.getStep(id)
  } yield transformation(id, step)
}

case class OptionProofView[F1, F2, R1, R2, B1, B2, Id](
  val inner: Proof[F1, R1, B1, Id],
  val transformation: (Id, Proof.Step[F1, R1, B1, Id]) => Option[Proof.Step[F2, R2, B2, Id]]
) extends Proof[F2, R2, B2, Id]
{
  def rootSteps = inner.rootSteps
  def getStep(id: Id): Option[Proof.Step[F2, R2, B2, Id]] = for {
    step <- inner.getStep(id)
    newStep <- transformation(id, step)
  } yield newStep
}
