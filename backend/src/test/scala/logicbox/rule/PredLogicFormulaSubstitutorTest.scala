...
class PredLogicFormulaSubstitutorTest extends AnyFunSpec {
  ...
  describe("substitute") {
    it("should substitute x with y inside predicate") {
      ...
      substitutor.substitute(f1, Var('x'), Var('y')) shouldBe exp1
      substitutor.substitute(f2, Var('x'), Var('y')) shouldBe exp2
    }
    ...
    it("should substitute inside connectives") {
      ...
      substitutor.substitute(f, Var('x'), Var('y')) shouldBe exp
    }

    it("should leave contr./taut. alone") {
      substitutor.substitute(Contradiction(), Var('x'), Var('y')) shouldBe Contradiction()
      substitutor.substitute(Tautology(), Var('x'), Var('y')) shouldBe Tautology()
    }
