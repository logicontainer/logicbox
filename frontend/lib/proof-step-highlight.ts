...
export function getStepHighlight(
  stepUuid: string,
  ...
  const refBeingEdited =
    interactionState.enum === InteractionStateEnum.EDITING_REF &&
    interactionState.lineUuid === stepUuid;
  const otherIsEditingRef =
    interactionState.enum === InteractionStateEnum.EDITING_REF &&
    interactionState.lineUuid !== stepUuid;
...
export function getDiagnosticHighlightForFormula(
  stepUuid: string,
  diagnosticContext: DiagnosticsContextProps,
) {
  ...
  return ds.length > 0 ? DiagnosticHighlight.YES : DiagnosticHighlight.NO;
}

function referenceIdxIsInDiagnostic(d: Diagnostic, refIdx: number): boolean {
  if (d.violationType !== d.violation.violationType) {
    throw new Error(
      `Violation types on diagnostic mismatched: ${JSON.stringify(d)}`,
    );
  }
...
export function getDiagnosticHighlightForReference(
  stepUuid: string,
  refIdx: number,
  diagnosticContext: DiagnosticsContextProps,
) {
...
export function getDiagnosticHighlightForRule(
  stepUuid: string,
  diagnosticContext: DiagnosticsContextProps,
) {
  const { diagnostics } = diagnosticContext;

  const ruleViolationTypes: ViolationType[] = ["missingRule"] as const;
