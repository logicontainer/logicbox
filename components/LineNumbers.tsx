  ...
  return (
    <div className="flex-col items-start">
      {lines.filter((line) => !line.isBox).map((line) => {
        const proofStepDiagnostics = proofDiagnostics.find((diagnostic) => diagnostic.uuid === line.uuid);
        return (
          <LineNumber key={line.uuid} line={line} proofStepDiagnostics={proofStepDiagnostics} />);
      })}
    </div>
  )
}